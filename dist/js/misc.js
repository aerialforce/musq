...
(function () {
    ...
    // extend Number As Vector, Matrix
    Object.extend(Array.prototype, {
        fill: function (v) {
        ...
        flatMap: function (f) {
            ...
            this.each(function (e) {
                if (e.flatMap)
                    result = result.concat(e.flatMap(f));
                else
        ...
        cross: function (that) {
            if (this.length === 2 && that.length === 2)
                return this[0] * that[1] - this[1] * that[0];
            else if (this.length === 3 && that.length === 3)
                return [
        ...
        rotate: function (radian) {
            if (this.length === 0)
                return this;
            else if (this[0] instanceof Array)
                return this.map(function (e) {
    ...
    ImageDataWrapper.prototype = {
        toByte: function (p) {
            if (p.length === 1)
                ...
                return (~~(p[0] * this.width + p[1] * this.height * this.width)) * 4;
        },
        at: function (p, c) {
            p = this.toByte(p);
            if (c) {
    ...
    DCore.prototype = {
        ...
        font: function (type, size, style) {
            if (type instanceof Array)
                return DCore.prototype.font.apply(this, type);
            if (typeof size === "number")
                size = this.toScr(size);
            this.ctx.font =
                    [(style) ? style : ""
        ...
        toScr: function (p) {
            ...
            // world -> screen
                ...
                return this.toScr(this.toWorld2d(p).slice(0, 2));
        },
        scr2World2d: function (p) {
            // scale value will accepted but not recommended
            if (!p.length)
                return (p - this.left2d) / this.width;
            // vector
            else
                return [
                    (p[0] - this.left2d) / this.width,
        ...
        // push matrix
        pushMatrix: function () {
            if (this.convMatrixStack.length > 64)
                throw "overload";
            this.convMatrixStack.push(this.convMatrix);
        },
        // pop matrix
        popMatrix: function () {
            if (this.convMatrixStack.length <= 0)
                throw "not loaded";
        ...
        transformTo: function (from, to, drawing) {
            // check
            if (!from)
                throw "from is abnormal";
            if (!to)
                throw "to is abnormal";
            if (from.length !== to.length)
                throw "there is different between from and to length";
            ...
            // a drawing method like GL_TRIANGLE_FUN
